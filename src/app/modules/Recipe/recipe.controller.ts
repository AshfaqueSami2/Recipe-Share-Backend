...
const createRecipe: RequestHandler = catchAsync(async (req, res) => {
  ...
  const imageUrls: string[] = imageBase64Array.length > 0
    ? await Promise.all(imageBase64Array.map((imageBase64: string) => uploadImageToImageBB(imageBase64)))
    : [];
  ...
  const populatedRecipe = await Recipe.findById(result._id).populate('user', 'name');
...
const deleteRecipe = catchAsync(async (req: Request, res: Response) => {
  ...
  const recipeOwnerId = recipe.user instanceof Types.ObjectId ? recipe.user.toString() : recipe.user._id?.toString();
  ...
  if (!isOwner && !isAdmin) {
    throw new AppError(httpStatus.FORBIDDEN, 'Unauthorized to delete this recipe');
  }
...
const rateRecipe = catchAsync(async (req: Request, res: Response) => {
  ...
  if (typeof rating !== 'number' || rating < 1 || rating > 5) {
    throw new AppError(httpStatus.BAD_REQUEST, 'Rating must be a number between 1 and 5');
  }
  ...
  const userId = Types.ObjectId.isValid(req.user.id) ? new Types.ObjectId(req.user.id) : req.user.id;
  ...
  recipe.averageRating = recipe.ratings.length ? totalRating / recipe.ratings.length : 0;
...
const upvoteRecipe = catchAsync(async (req: Request, res: Response) => {
  ...
  recipe.downvotes = recipe.downvotes.filter((downvote) => !downvote.equals(userId));
...
const downvoteRecipe = catchAsync(async (req: Request, res: Response) => {
  ...
  const hasDownvoted = recipe.downvotes.some((downvote) => downvote.equals(userId));
  if (!hasDownvoted) {
    // Add the downvote
